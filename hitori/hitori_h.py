...
class Hitori_h():
    ...
    def solve(self):
        ...
        if self.isSolved():
            return self.grid
        result = Hitori_h(self.size, self.grid).solve()
        if result:
            return result
        else:
            if self.grid[r][c].shade == 1:
                return False
            else:
                self.grid = deepcopy(self.init_grid)
                self.grid[r][c].shade = 1
                ...
                if self.isSolved():
                    return self.grid
                return Hitori_h(self.size, self.grid).solve()
